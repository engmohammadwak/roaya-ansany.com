    ...
    <div class="container">
        <div class="donation-counter-box">
            ...
            <div class="text-center mb-5">
                <h6 class="main-color">{{ $locale === 'ar' ? 'عداد التبرعات' : 'Donation Counter' }}</h6>
                <h2 class="section-title">
                    {{ $locale === 'ar' ? 'معاً نصنع الفرق' : 'Together We Make a Difference' }}
                </h2>
            </div>
            ...
            <div class="row text-center mb-4 g-4">
                <div class="col-md-4">
                    <div class="counter-stat-card">
                        ...
                        <div class="counter-number" data-target="{{ $raised }}" data-currency="{{ $curr }}">{{ $curr }}0</div>
                        <div class="counter-label">{{ $locale === 'ar' ? 'تم جمعه' : 'Raised' }}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="counter-stat-card featured">
                        <div class="counter-icon"><i class="fa-solid fa-bullseye"></i></div>
                        <div class="counter-number" data-target="{{ $goal }}" data-currency="{{ $curr }}">{{ $curr }}0</div>
                        <div class="counter-label">{{ $locale === 'ar' ? 'الهدف الكلي' : 'Total Goal' }}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="counter-stat-card">
                        <div class="counter-icon"><i class="fa-solid fa-hourglass-half"></i></div>
                        <div class="counter-number" data-target="{{ $remaining }}" data-currency="{{ $curr }}">{{ $curr }}0</div>
                        <div class="counter-label">{{ $locale === 'ar' ? 'المتبقي' : 'Remaining' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="counter-progress-wrap mb-2">
                <div class="counter-progress-bar">
                    <div class="counter-progress-fill" style="width: 0%" data-width="{{ $pct }}"></div>
                </div>
                <div class="d-flex justify-content-between mt-2">
                    <span class="counter-pct-label main-color fw-bold">0%</span>
                    <span class="color-67" style="font-size:13px">{{ $pct }}% {{ $locale === 'ar' ? 'مكتمل' : 'Complete' }}</span>
                </div>
            </div>

            {{-- CTA --}}
            <div class="text-center mt-5">
                <a href="{{ url($locale.'/donate') }}" class="btn-donate px-5">
                    <i class="fa-solid fa-heart me-2"></i>
                    {{ $locale === 'ar' ? 'تبرع الآن واصنع الفرق' : 'Donate Now & Make a Difference' }}
                </a>
            </div>
        </div>
    </div>
