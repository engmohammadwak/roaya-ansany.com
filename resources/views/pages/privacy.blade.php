...
@section('title', ($isAr ? 'سياسة الخصوصية' : 'Privacy Policy') . ' | مؤسسة رؤيا الإنسانية')
@push('styles')
<style>
...
<div class="breadcrumb-section">
    <div class="container"><small><a href="{{ url($locale.'/') }}">{{ $isAr?'الرئيسية':'Home' }}</a><span style="margin:0 8px;color:#888">›</span><span class="text-muted">{{ $isAr?'سياسة الخصوصية':'Privacy Policy' }}</span></small></div>
</div>
<section class="policy-hero">
    <div class="container">
        <h1>{{ $isAr ? ($privacy?->title_ar ?? 'سياسة الخصوصية') : ($privacy?->title_en ?? 'Privacy Policy') }}</h1>
    </div>
</section>
<section class="policy-section">
    <div class="container">
        <div class="policy-content">
            @if($privacy?->last_updated)
            <div class="policy-updated">{{ $isAr?'آخر تحديث:':'Last updated:' }} {{ $privacy->last_updated->format('Y-m-d') }}</div>
            @endif
            @if($privacy)
                {!! $isAr ? $privacy->content_ar : $privacy->content_en !!}
            @else
